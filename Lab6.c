...
void heapWalk( char * caption)
{
	...
	do
	{
		...
		printf("%d\t%d\t%d bytes \t8 bytes\t%d\n",((int)blockstart),baseoffset,payload,blocksize);
		...
		// 8: increment baseoffset by this block's blocksize  (int + int i.e. add blocksize to baseoffset i.e.  )
